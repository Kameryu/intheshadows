...
public class Move2 : MonoBehaviour
{
	...
	void Start()
	{
		...
		myVal.x = 45;
		myVal.y = -135;
		myObject1.transform.position = new Vector3(2, 5, -2.5f);
	}
	...
	void Update()
	{
		...
		bool ok2 = myObject2.GetComponent<Move4>().isOk;
		if (isOk && ok2 && Mathf.Abs(pos2.y - pos1.y) < 1)
			affVictory();
	...
	void checkOk()
	{
		...
		if (Mathf.Abs(angle2.y - baseRot.y) < 3.5f)
			...
			if (Mathf.Abs(angle3.x - baseRot.x) < 3.5f)
				isOk = true;
			else
				isOk = false;
		else
